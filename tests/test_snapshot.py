...
from aegis_isle.core.state.snapshot import SnapshotManager, StateSnapshot
...
class TestSnapshotSystem:
    ...
    async def test_create_snapshot(self, temp_snapshot_dir):
        """测试创建快照"""
        # 初始化
        state_manager = StateManager(state_dir=temp_snapshot_dir.replace("snapshots", "state"))
    ...
    async def test_rollback(self, temp_snapshot_dir):
        ...
        # 回滚
        restored_state = await snapshot_manager.rollback_to_snapshot(user_id, snapshot1.snapshot_id)
    ...
    async def test_cleanup_old_snapshots(self, temp_snapshot_dir):
        ...
        assert len(snapshots_before) == 15, f"应该有 15 个快照,实际有 {len(snapshots_before)} 个"
        ...
        assert len(snapshots_after) == 10, f"应该剩余 10 个快照,实际剩余 {len(snapshots_after)} 个"
