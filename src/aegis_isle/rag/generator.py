...
class LLMGenerator(BaseGenerator):
    ...
    def __init__(self, config: GenerationConfig, provider: str = "openai"):
        ...
        try:
            if self.provider == "openai":
                ...
                if settings.openai_base_url:
                    ...
                    logger.info(
                        f"Using custom OpenAI base URL: {settings.openai_base_url}"
                    )
            ...
            result.metadata.update(
                {
                    ...
                    "has_context": bool(
                        retrieval_context and retrieval_context.results
                    ),
                    "context_chunks": len(retrieval_context.results)
    ...
    async def generate_stream(
        ...
        **kwargs,
        ...
        logger.info(
            f"Calling LLM with provider={self.provider}, model={generation_config.get('model')}"
        )
        ...
        if self.provider == "openai":
            ...
            kwargs_copy.pop(
                "stream", None
            )  # Remove stream arg as we use stream() method
            # Adjust parameter names
            if "model" in kwargs_copy:
                kwargs_copy["model"] = kwargs_copy["model"].replace("gpt", "claude")

            async with self._client.messages.stream(
                messages=[{"role": "user", "content": enhanced_prompt}], **kwargs_copy
            ) as stream:
    ...
    async def generate_with_context(
        ...
        for i, chunk_text in enumerate(context_chunks):
            chunk = DocumentChunk(
                document_id="provided_context", content=chunk_text, chunk_index=i
            )
            result = RetrievalResult(chunk=chunk, score=1.0)
            mock_results.append(result)

        context = EnhancedQueryResult(query=query, results=mock_results, total_time=0.0)
...
Answer:"""
    ...
    async def _generate_openai(self, prompt: str, **kwargs) -> GenerationResult:
        ...
        try:
            ...
            response = await self._client.chat.completions.create(
                messages=[{"role": "user", "content": prompt}], **generation_config
            )
    ...
    async def _generate_anthropic(self, prompt: str, **kwargs) -> GenerationResult:
        ...
        try:
            ...
            generation_config = {
                "model": kwargs.get(
                    "model", self.config.model.replace("gpt", "claude")
                ),
                "max_tokens": kwargs.get("max_tokens", self.config.max_tokens),
                "temperature": kwargs.get("temperature", self.config.temperature),
                "top_p": kwargs.get("top_p", self.config.top_p),
    ...
    async def generate(
        ...
        **kwargs,
        ...
        try:
            ...
            # Tokenize input
            inputs = self._tokenizer.encode(enhanced_prompt, return_tensors="pt").to(
                self.device
            )
