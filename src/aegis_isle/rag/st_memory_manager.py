...
class STMemoryManager:
        ...
        class EmbedderWrapper:
            ...
            def embed_documents(self, texts: List[str]) -> List[List[float]]:
                ...
                raise NotImplementedError(
                    "Use async FAISS or sync wrapper for document embedding"
                )
    ...
    def _get_index_path(
        self, character_name: str, world_line: Optional[str] = None
    ) -> str:
        import re
        ...
        if os.path.exists(index_path):
            try:
                ...
                vectorstore = FAISS.load_local(
                    temp_path, self.embedder, allow_dangerous_deserialization=True
                )
    ...
    def ingest_chunks(
        ...
        if vectorstore:
            ...
            logger.info(
                f"Added {len(documents)} chunks to existing index for {character_name}"
            )
        else:
            # Create new index
            ...
            self.indices[index_key] = vectorstore
            logger.info(
                f"Created new index for {character_name} with {len(documents)} chunks"
            )
        ...
        try:
            ...
            try:
                ...
                asyncio.run_coroutine_threadsafe(
                    event_bus.log_chat_summary(
                        world_line or "default", character_name, summary
                    ),
                    loop,
                )
            except RuntimeError:
    ...
    def _fetch_scene_meta(self, parent_chunk_id: str, chat_file: str) -> dict:
        ...
        search_pattern = os.path.join(
            "debug", "chunks", f"{prefix}_parent_chunks.jsonl"
        )
    ...
    def _post_filter_by_metadata(
        self, docs: List[Document], hints: List[str]
    ) -> List[Document]:
        """
        ...
        logger.info(
            f"[Memory] 元数据过滤: hints={hints}, matched={len(matched)}, unmatched={len(unmatched)}"
        )
    ...
    def _fetch_episode_plot(self, parent_chunk_id: str, chat_file: str) -> str:
        ...
        ep_files = glob.glob(
            os.path.join("debug", "chunks", f"*{world_line}*_episodes.jsonl")
        )
    ...
    def _fetch_parent_chunk_text(
        self, parent_chunk_id: str, chat_file: str, sub_chunk_text: str = ""
    ) -> str:
                        ...
                        if parent_chunk_id in line:
                            ...
                            if data.get("parent_chunk_id") == parent_chunk_id:
                                ...
                                if sub_chunk_text and sub_chunk_text in full_text:
                                    ...
                                    text_preview = (
                                        full_text[:WINDOW_SIZE] + "…（场景略）"
                                    )
                                else:
                                    text_preview = full_text

                                # ── 获取 episode plot 摘要 ──
                                plot = self._fetch_episode_plot(
                                    parent_chunk_id, chat_file
                                )
    ...
    def format_context_for_prompt(
        self, documents: List[Document], max_chunks: int = 3
    ) -> str:
        """
        ...
        for i, doc in enumerate(documents, 1):
            ...
            full_context = (
                self._fetch_parent_chunk_text(
                    parent_id, chat_file, sub_chunk_text=doc.page_content
                )
                if parent_id
                else ""
            )
            ...
            formatted_chunks.append(
                f"【记忆片段 {len(formatted_chunks) + 1}】[所属宇宙: {universe_label}]（来源：{chat_file}）\n{content}"
            )
