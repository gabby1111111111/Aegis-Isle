...
class DailyDigest:
    ...
    def __init__(
        self,
        events_dir: str = "data/diary/events",
        digests_dir: str = "data/diary/digests",
        vectorstore_dir: str = "data/vectorstore/diary",
    ):
    ...
    def collect_events(self) -> str:
        ...
        if interview_events:
            ...
            sections.append(
                f"- 今日共练习 {total} 道题目，正确 {correct} 题 (正确率: {int(correct / total * 100) if total else 0}%)"
            )
    ...
    async def compile_and_index(self):
        ...
        def _index():
            if os.path.exists(self.index_path):
                ...
                logger.info(
                    f"Appended daily diary to existing FAISS index: {self.index_path}"
                )
            else:
