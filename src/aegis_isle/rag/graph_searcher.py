...
class JsonlGraphSearcher(BaseGraphSearcher):
    ...
    def _load_universe_graph(self, universe_id: str):
        ...
        logger.info(
            f"[GraphSearcher] 宇宙 {universe_id} - 已加载 {len(nodes)} 节点, {len(edges)} 边"
        )
    ...
    async def search(self, query: str, universe_id: str, character_name: str) -> str:
        ...
        safe_world = "".join(
            [c for c in universe_id if c.isalnum() or c in (" ", "-", "_")]
        ).strip()
        ...
        related_edges = [
            e
            for e in edges
            if e.get("source") == target_node.get("node_id")
            or e.get("target") == target_node.get("node_id")
        ]
        edge_strs = []
