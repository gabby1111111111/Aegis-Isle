...
async def search_memory(req: MemorySearchRequest):
    ...
    """
    logger.info(
        f"[Memory] 查询 {req.character_name} 的记忆, query='{req.query[:30]}...'"
    )
    ...
    try:
        ...
        async def _run_faiss():
            ...
            docs = await memory_manager.search_memory(
                req.query, req.character_name, req.world_line, req.k
            )
            return docs
        ...
        async def _run_graph():
            if not do_graph:
                return ""
            return await graph_searcher.search(
                req.query, req.world_line or "", req.character_name
            )
        ...
        docs, graph_text, episode_text, diary_text = await asyncio.gather(
            _run_faiss(), _run_graph(), _run_episode(), _run_diary()
        )
        ...
        final_context_parts = []
        if diary_text:
            final_context_parts.append(diary_text)
        if graph_text:
            final_context_parts.append(graph_text)
        if episode_text:
            final_context_parts.append(episode_text)
        if faiss_text:
            final_context_parts.append(f"【最近聊天片段】\n{faiss_text}")
        ...
        logger.info(
            f"[Memory] 并发查询完成, context总长度: {len(final_context_string)}"
        )
...
async def debug_save_prompt(req: DebugSaveRequest):
    ...
    try:
        ...
        safe_world = "".join(
            [c for c in req.universe_id if c.isalnum() or c in (" ", "-", "_")]
        ).strip()
...
async def ingest_memory(req: MemoryIngestRequest):
    ...
    logger.info(
        f"[Memory] 接收新对话片段用于存入 {req.character_name} 的记忆库, 共 {len(req.messages)} 条消息"
    )

    try:
        if len(req.messages) < 2:
            return JSONResponse(
                {"status": "skipped", "reason": "消息数量太少，不值得存入"}
            )
        ...
        return JSONResponse(
            {"status": "ok", "message": f"已将 {len(req.messages)} 条消息存入记忆库"}
        )
...
async def receive_diary_event(req: DiaryEventRequest):
    ...
    try:
        ...
        if source == "browsing":
            ...
            return JSONResponse(
                status_code=400,
                content={"status": "error", "message": f"未知来源: {source}"},
            )
