...
class MemoryEntry(Base):
    ...
    meta_data = Column(
        Text
    )  # JSON string (renamed from metadata to avoid SQLAlchemy reserved word)
...
class AgentMemory:
    ...
    def retrieve_recent_messages(
        self, agent_id: str, limit: int = 10, include_system: bool = True
    ) -> List[AgentMessage]:
        ...
        try:
            ...
            if agent_id in self._cache and len(self._cache[agent_id]) >= limit:
                ...
                if include_system or not any(
                    entry.is_system for entry in cached_entries
                ):
            ...
            if not include_system:
                query = query.filter(not MemoryEntry.is_system)
    ...
    def retrieve_important_messages(
        self, agent_id: str, min_importance: int = 5, limit: int = 20
    ) -> List[AgentMessage]:
        ...
        try:
            entries = (
                ...
                .order_by(
                    MemoryEntry.importance_score.desc(), MemoryEntry.created_at.desc()
                )
                .limit(limit)
                .all()
            )
            ...
            logger.error(
                f"Error retrieving important messages for agent {agent_id}: {e}"
            )
            return []
    ...
    def update_importance_score(
        self, agent_id: str, message_id: str, new_score: int
    ) -> bool:
        ...
        try:
            ...
            if entry:
                ...
                self.session.commit()
                logger.debug(
                    f"Updated importance score for message {message_id} to {new_score}"
                )
                return True
            ...
            if agent_id in self._cache:
                if keep_important:
                    self._cache[agent_id] = [
                        entry
                        for entry in self._cache[agent_id]
    ...
    def cleanup_old_memories(self, days_to_keep: int = 30) -> int:
        ...
        try:
            ...
            self.session.query(MemoryEntry).filter(
                MemoryEntry.created_at < cutoff_date, MemoryEntry.importance_score < 5
            ).delete()
    ...
    def get_memory_stats(self, agent_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            important_entries = (
                self.session.query(MemoryEntry)
                .filter(
                    MemoryEntry.agent_id == agent_id, MemoryEntry.importance_score >= 5
                )
                .count()
            )
