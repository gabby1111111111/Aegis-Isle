...
def clean_st_text(text: str) -> str:
    ...
    cutoff_pattern = (
        r"(当前bgm:|⋯♡⋯|```html|```mermaid|𐙚₊˚|☆₊⁺|𓋫 𓏴𓏴|【小剧场|剧情分支).*?(\n|$)"
    )
...
def parse_st_chat_log(
    file_path: str, character_name: str, world_line: str = None, chunk_size: int = 1
) -> List[ChatChunk]:
    """
        ...
        if msg["role"] == "char":
            ...
            if len(ai_text) > 800:
                ...
                for para in paragraphs:
                    ...
                    if len(current_sub_chunk_text) + len(para) > 1000 and len(
                        current_sub_chunk_text
                    ) > len(base_context + f"{msg['name']}: "):
                        chunks.append(
                            ChatChunk(
...
def main():
    parser = argparse.ArgumentParser(
        description="Ingest SillyTavern JSONL chat logs into Aegis-Isle ST Memory FAISS Index."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The name of the character (e.g., 邹峥)",
    )
    parser.add_argument(
        "--world-line",
        type=str,
        default=None,
        help="Optional world line namespace to group chats together (e.g., AIDom项目宇宙)",
    )
    parser.add_argument(
        "--chunk-size",
        type=int,
        default=6,
        help="Number of conversational turns per chunk",
    )
    ...
    chunks = parse_st_chat_log(
        args.file_path, args.character, args.world_line, args.chunk_size
    )
