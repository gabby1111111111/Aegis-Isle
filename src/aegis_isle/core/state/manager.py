...
class StateManager:
    ...
    async def load_state(self, user_id: str) -> UserState:
        ...
        async with self._get_lock(user_id):
            ...
            try:
                ...
                logger.info(f"Loaded state for user {user_id} with {len(state.sheets)} sheets")
                return state
    ...
    def _create_default_state(self, user_id: str) -> UserState:
        ...
        default_sheets = {
            ...
            "sheet_hero": Sheet(
                ...
                content=[
                    [None, "人物名称", "性别/年龄", "外貌特征", "职业/身份", "过往经历", "性格特点"]
                ],
                orderNo=1
            ),
            ...
            "sheet_quest": Sheet(
                ...
                content=[
                    [None, "任务名称", "任务类型", "发布者", "详细描述", "当前进度", "任务时限", "奖励", "惩罚"]
                ],
                orderNo=5
            ),
        }
    ...
    async def apply_edits(
        self,
        state: UserState,
        commands: List[TableEditCommand]
    ) -> UserState:
        """
                ...
                if cmd.edit_type == EditType.INSERT:
                    ...
                    if cmd.condition and cmd.row_data:
                        ...
                        for i, row in enumerate(sheet.get_rows(), start=1):
                            if len(row) > col_idx and row[col_idx] == target_value:
                                ...
                                logger.info(f"Updated row in '{cmd.sheet_name}' (matched: {target_value})")
                                updated = True
                                break
                    ...
                    if cmd.condition:
                        ...
                        for row in sheet.get_rows():
                            if not (len(row) > col_idx and row[col_idx] == target_value):
