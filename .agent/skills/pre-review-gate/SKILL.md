...
## 🎮 Gabby 大人验收指南 (Human Review Guide)

为了让 Gabby 大人在 30 秒内完成体验，请提供最傻瓜式的指令（可直接复制粘贴）：

**1. 怎么起服务？**
```bash
# 请 Agent 在这里写明启动服务的精准命令，带上绝对/相对路径和必要参数
# 例: cd E:\universe_manager && streamlit run app.py
```

**2. 怎么点？**
- 第一步：打开浏览器访问 `http://localhost:XXXX`
- 第二步：点击左侧的 `[某个按钮]`
- 第三步：输入 `[某个测试数据]`

**3. 怎么算验收通过？**
- 预期现象：看到什么画面 / 数据库里多出什么字段。
- 如果出现了 XXX，说明有问题，请打回。
