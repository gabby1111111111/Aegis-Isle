...
Gate 4  浏览器实测 + 截图录屏 ──── 视觉层 (本 Skill 新增)
...
2. 使用 `browser_subagent` 工具:
   - 打开页面 URL
   - **录屏**操作核心交互流程（点按钮、输入数据、切换 Tab 等）
   - 录屏会自动保存为 `.webp` 到 artifacts 目录
   - 额外截图关键状态页面
3. 验证:
   - 页面无报错红框
   - 核心功能按钮可点击且有响应
...
3. 下次提交时 Gate 4 必须包含**至少 2 张截图 + 1 段录屏**
